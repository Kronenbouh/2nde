...
\pagestyle{fancy}
\fancyhead[L]{Seconde}
\fancyhead[C]{\textbf{DS n°2 — Plan cartésien}}
\fancyhead[R]{\today}
...
\item Un repère est un système de trois $\dots$
\item Dans un repère orthogonal les axes sont $\dots$
\end{enumerate}
}{exe:1}{
Voir cours
}
...
On considère le point $H(\sqrt{2} ; 2)$. 
...
\begin{center}
	\begin{tikzpicture}[>=stealth, scale=.8]
		\begin{axis}[xmin = 1, xmax=2, ymin=-0.1, ymax=3, axis x line=middle, axis y line=middle, axis line style=<->, xlabel={}, ylabel={}, grid=both, grid style = {opacity=.5}, clip=false, xtick distance = 0.1, ytick distance=1, x=10cm, y=1cm]
		\addplot[black, mark=*, mark size = 1] (1.4,2) node[above right] {$E$};
		\addplot[black, mark=*, mark size = 1] (1.5,2) node[above right] {$F$};
		\end{axis}
	\end{tikzpicture}
\end{center}
}
...
\item .Donner les \textbf{plus petits} ensembles de nombres auxquels $x_M$ et $y_M$ appartiennent.
\end{enumerate}
\textit{Remarque : ``plus petit'' est entendu au sens de l'inclusion}
}{exe:3}{
\begin{enumerate}
\item $AB^2 = (x_B-x_A)^2+(y_B-y_A)^2=1+4=5$. Donc $AB=\sqrt{5}$.
\item \[ \left \{ \def\arraystretch{2.2} \begin{array}{c c c c c}
x_M & = & \frac{4 + 3}{2} & = & \dfrac72 \\
y_M & = & \frac{\dfrac23 + (-\dfrac43)}{2} & = & - \dfrac13 \\
\end{array} \right. \]
\item $x_M \in \D$ et $y_M \in \Q$.
\end{enumerate}
}
...
\end{multicols}
	...
	\begin{enumerate}
		...
		\begin{enumerate}[label=\roman*)]
			...
			\item $BC^2 = 292$
		\end{enumerate}
		\item $AB^2=AC^2+BC^2$ donc, d'après la réciproque du théorème de Pythagore, le triangle est rectangle en $C$.
	\end{enumerate}
}
%	\begin{center}
...
\exe{4}{
On considère les points $A \left ( -\frac{3}{2} ; 4 \right )$ et $B \left ( 2 ; 1 \right)$. 
Calculer les coordonnées du point $C$ afin que $B$ soit le milieu du segment $[AC]$
}{exe:5}{
On doit résoudre $2 = \dfrac{-\frac{3}{2} + x_C}2$ soit $x_C = 4 + \frac{3}{2} = \frac{11}{2}$. 
On fait de même avec l'ordonnée et on trouve $y_C = -2$.
}

\exe{}{
(Exercice bonus) \\
Donner deux points $A$ et $B$ dont toutes les coordonnées sont des \textbf{nombres réels non rationnels} 
et tels que, les coordonnées du point $M$, milieu du segment $[AB]$, sont des \textbf{nombres entiers}.
}{exe:6}{
On prend par exemple : $A(\sqrt{2}, \sqrt{3})$ et $B(-\sqrt{2}, -\sqrt{3})$.
}
